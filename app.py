...
# Ensure required session state variables exist
    ...
    if 'X_test' in st.session_state and 'model' in st.session_state:
        try:
            ...
            rmse = mean_squared_error(st.session_state.y_test, y_pred) ** 0.5  # ✅ Manually compute RMSE
...
with tabs[6]:  # Download Report
    ...
    model = st.session_state.trained_model
    model_option = st.session_state.model_option if "model_option" in st.session_state else "Unknown Model"
    ...
    report_content = ""  # Store formatted report text
    if include_summary:
